...
from public.teams h, public.teams a where h.name = 'Amatori Calcio Genova' and a.name = 'Sezione 164';

insert into public.matches (home_team_id, away_team_id, match_date, round, status, home_score, away_score)
select h.id, a.id, '2026-05-21 21:00:00+02'::timestamptz, 'Girone A', 'TERMINATA', 2, 2
from public.teams h, public.teams a where h.name = 'Aston Birra' and a.name = 'UCG (Bairon)';
...
from public.teams h, public.teams a where h.name = 'Lo Dico FC' and a.name = 'chainz';
...
from public.teams h, public.teams a where h.name = 'Tama' and a.name = 'Sezione 164';
...
from public.teams h, public.teams a where h.name = 'UCG (Bairon)' and a.name = 'Lo Dico FC';
